...
export default function Nav() {
  ...
  function handleSectionClick(href: string, e: React.MouseEvent) {
    e.preventDefault()
    setMenuOpen(false)
    if (isHome) {
    ...
    <>
      ...
      >
          ...
          {navItems.map((l) => (
            <li key={l.href}>
              {l.route ? (
                ...
                <a
                  href={l.href}
                  onClick={(e) => handleSectionClick(l.href, e)}
                  className={`nav-link${active === '#' + l.section ? ' nav-link--active' : ''}`}
                >
                  {l.label}
                  {active === '#' + l.section && (
        ...
        <div className="nav-actions">
          <Link to="/resume" className="nav-resume" aria-label="View Résumé">
            <FileText size={14} />
            <span>Résumé</span>
          </Link>
          <a
